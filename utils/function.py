...
from utils.utils import get_confusion_matrix, get_confusion_matrix1
...
def train(config, epoch, num_epoch, epoch_iters, base_lr,
    ...
    # for i_iter, batch in enumerate(next(iter(trainloader)), 0):
...
def validate(config, testloader, model, writer_dict):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(testloader, total=len(testloader))):
            ...
            for i, x in enumerate(pred):
                ...
                confusion_matrix[..., i] += get_confusion_matrix1(
                    label,
                    x,
                    size,
...
def testval1(testloader, model):
    ...
    with torch.no_grad():
        for idx, batch in enumerate(tqdm(testloader, total=len(testloader))):
            ...
            confusion_matrix += get_confusion_matrix1(
                    label,
                    pred,
                    size,
                    19,
    ...
    mean_acc = (tp / np.maximum(1.0, pos)).mean()
    mean_IoU = (IoU_array).mean()
    iou_array = [iou.item() for iou in IoU_array]
    print('Mean IoU: {:.4f} || IoU per class {}'.format(mean_IoU, iou_array))
