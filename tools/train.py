...
def main():
    ...
    if config.TRAIN.RESUME:
        model_state_file = os.path.join(final_output_dir, 'back_up_checkpoint.pth.tar')
    ...
    for epoch in range(last_epoch, end_epoch):
        ...
        #save back_up checkpoint
        torch.save({
            'epoch': epoch + 1,
            'best_mIoU': best_mIoU,
            'state_dict': model.model.state_dict(),
            'optimizer': optimizer.state_dict(),
        }, os.path.join(final_output_dir, 'back_up_checkpoint.pth.tar'))
        ...
        logger.info('Training time: {}'.format(str(datetime.timedelta(seconds=(end - start)))))
    torch.save(model.model.state_dict(),
